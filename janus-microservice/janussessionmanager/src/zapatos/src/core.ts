...
import type { Updatable, Whereable, Table, Column } from '../schema';
...
export class ParentColumn {
  constructor(public value: Column) {}
}
/**
 ...
 */
export function parent(x: Column) {
  return new ParentColumn(x);
}

export type GenericSQLExpression =
  | SQLFragment<any>
  | Parameter
  | DefaultType
  | DangerousRawString
...
>(literals: TemplateStringsArray, ...expressions: InferredInterpolations[]) {
  return new SQLFragment<RunResult>(
    Array.prototype.slice.apply(literals),
    expressions
  );
}
...
export class SQLFragment<RunResult = pg.QueryResult['rows']> {
  ...
  compileExpression = (
    ...
    if (expression instanceof SQLFragment) {
      ...
      result.text +=
        expression.charAt(0) === '"' ? expression : `"${expression}"`;
      ...
      const columnNames = Array.isArray(expression.value)
        ? expression.value
      ...
      if (columnNames.length) {
        ...
        for (let i = 0, len = columnNames.length; i < len; i++) {
          ...
          if (columnValue instanceof SQLFragment) {
            ...
            this.compileExpression(
              columnValue instanceof ParentColumn
                ? columnValue
                : new Parameter(columnValue),
              result,
              parentTable,
