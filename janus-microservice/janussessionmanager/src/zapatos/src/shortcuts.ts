...
type JSONSelectableForTable<T extends Table> = {
  [K in keyof SelectableForTable<T>]: Date extends SelectableForTable<T>[K]
    ? Exclude<SelectableForTable<T>[K], Date> | DateString
    : Date[] extends SelectableForTable<T>[K]
    ? Exclude<SelectableForTable<T>[K], Date[]> | DateString[]
...
interface InsertSignatures {
  <T extends Table>(table: T, values: InsertableForTable<T>): SQLFragment<
...
export const insert: InsertSignatures = function (
  ...
  const completedValues = Array.isArray(values)
      ? completeKeysWithDefault(values)
      : values,
    colsSQL = cols(
      Array.isArray(completedValues) ? completedValues[0] : completedValues
    ),
    valuesSQL = Array.isArray(completedValues)
      ? mapWithSeparator(
          completedValues as Insertable[],
          sql<SQL>`, `,
          (v) => sql<SQL>`(${vals(v)})`
        )
    ...
    query = sql<
      SQL
    >`INSERT INTO ${table} (${colsSQL}) VALUES ${valuesSQL} RETURNING to_jsonb(${table}.*) AS result`;
...
type UpsertReturnableForTable<T extends Table> = JSONSelectableForTable<T> &
  UpsertAction;
type UpsertConflictTargetForTable<T extends Table> =
  | Constraint<T>
  | ColumnForTable<T>
  | ColumnForTable<T>[];
...
interface UpsertSignatures {
  <T extends Table>(
    ...
    noNullUpdateCols?: ColumnForTable<T> | ColumnForTable<T>[]
  ): SQLFragment<UpsertReturnableForTable<T>>;
  <T extends Table>(
    table: T,
    values: InsertableForTable<T>[],
    conflictTarget: UpsertConflictTargetForTable<T>,
...
export const upsert: UpsertSignatures = function (
  ...
  noNullUpdateCols: Column | Column[] = []
): SQLFragment<any> {
  ...
  const completedValues = Array.isArray(values)
      ? completeKeysWithDefault(values)
      : values,
    firstRow = Array.isArray(completedValues)
      ? completedValues[0]
      : completedValues,
    ...
    uniqueColsSQL = Array.isArray(conflictTarget)
      ? sql`(${mapWithSeparator(
          conflictTarget.slice().sort(),
          sql`, `,
          (c) => c
        )})`
      : sql<string>`ON CONSTRAINT ${conflictTarget.value}`,
    updateColsSQL = mapWithSeparator(
      nonUniqueCols.slice().sort(),
      sql`, `,
      (c) => c
    ),
    updateValuesSQL = mapWithSeparator(
      nonUniqueCols.slice().sort(),
      sql`, `,
      (c) =>
        noNullUpdateCols.includes(c)
  ...
  const query = sql<
    SQL
  >`INSERT INTO ${table} (${colsSQL}) VALUES ${valuesSQL} ON CONFLICT ${uniqueColsSQL} DO UPDATE SET (${updateColsSQL}) = ROW(${updateValuesSQL}) RETURNING to_jsonb(${table}.*) || jsonb_build_object('$action', CASE xmax WHEN 0 THEN 'INSERT' ELSE 'UPDATE' END) AS result`;
...
interface UpdateSignatures {
  ...
  ): SQLFragment<JSONSelectableForTable<T>[]>;
}
...
export const update: UpdateSignatures = function (
  ...
  const query = sql<SQL>`UPDATE ${table} SET (${cols(values)}) = ROW(${vals(
    values
  )}) WHERE ${where} RETURNING to_jsonb(${table}.*) AS result`;
...
export const deletes: DeleteSignatures = function (
  ...
  const query = sql<
    SQL
  >`DELETE FROM ${table} WHERE ${where} RETURNING to_jsonb(${table}.*) AS result`;
  query.runResultTransform = (qr) => qr.rows.map((r) => r.result);
  return query;
};
...
interface TruncateSignatures {
  ...
  (table: Table | Table[], optFK: TruncateForeignKeyOpts): SQLFragment<
    undefined
  >;
  (
    table: Table | Table[],
    optId: TruncateIdentityOpts,
    optFK: TruncateForeignKeyOpts
...
export const truncate: TruncateSignatures = function (
  ...
  const tables = mapWithSeparator(table, sql`, `, (t) => t),
    query = sql<SQL, undefined>`TRUNCATE ${tables}${raw(
...
export type PromisedSQLFragmentReturnType<
  R extends SQLFragment<any>
> = PromisedType<ReturnType<R['run']>>;
export type PromisedSQLFragmentReturnTypeMap<L extends SQLFragmentsMap> = {
  [K in keyof L]: PromisedSQLFragmentReturnType<L[K]>;
};

export type JSONOnlyColsForTable<
  T extends Table,
  C extends any[] /* TS can't manage being more specific here */
> = Pick<JSONSelectableForTable<T>, C[number]>;

type BaseSelectReturnTypeForTable<
  T extends Table,
  C extends ColumnForTable<T>[]
> = C extends undefined
  ? JSONSelectableForTable<T>
...
> = L extends undefined
  ? E extends undefined
    ? BaseSelectReturnTypeForTable<T, C>
    : BaseSelectReturnTypeForTable<T, C> & PromisedSQLFragmentReturnTypeMap<E>
  : E extends undefined
  ? BaseSelectReturnTypeForTable<T, C> & PromisedSQLFragmentReturnTypeMap<L>
  : BaseSelectReturnTypeForTable<T, C> &
      PromisedSQLFragmentReturnTypeMap<L> &
      PromisedSQLFragmentReturnTypeMap<E>;
...
> = M extends SelectResultMode.Many
  ? EnhancedSelectReturnTypeForTable<T, C, L, E>[]
  : M extends SelectResultMode.One
  ? EnhancedSelectReturnTypeForTable<T, C, L, E> | undefined
  : number;
...
export const select: SelectSignatures = function (
  ...
  const allOptions =
      mode === SelectResultMode.One ? { ...options, limit: 1 } : options,
    aliasedTable = allOptions.alias || table,
    lateralOpt = allOptions.lateral,
    extrasOpt = allOptions.extras,
    tableAliasSQL =
      aliasedTable === table ? [] : sql<string>` AS ${aliasedTable}`,
    colsSQL =
      mode === SelectResultMode.Count
        ? allOptions.columns
          ? sql`count(${cols(allOptions.columns)})`
          : sql<typeof aliasedTable>`count(${aliasedTable}.*)`
        : allOptions.columns
        ? sql`jsonb_build_object(${mapWithSeparator(
            allOptions.columns,
            sql`, `,
            (c) => sql<SQL>`${param(c)}::text, ${c}`
          )})`
        : sql<typeof aliasedTable>`to_jsonb(${aliasedTable}.*)`,
    colsLateralSQL =
      lateralOpt === undefined
        ? []
        : sql` || jsonb_build_object(${mapWithSeparator(
            Object.keys(lateralOpt),
            sql`, `,
            (k, i) =>
              sql<SQL>`${param(k)}::text, "ljoin_${raw(String(i))}".result`
          )})`,
    colsExtraSQL =
      extrasOpt === undefined
        ? []
        : sql<any[]>` || jsonb_build_object(${mapWithSeparator(
            Object.keys(extrasOpt),
            sql`, `,
            (k) => sql<SQL>`${param(k)}::text, ${extrasOpt![k]}`
          )})`,
    ...
    limitSQL =
      allOptions.limit === undefined
        ? []
        : sql` LIMIT ${param(allOptions.limit)}`,
    offsetSQL =
      allOptions.offset === undefined
        ? []
        : sql` OFFSET ${param(allOptions.offset)}`,
    lateralSQL =
      lateralOpt === undefined
        ? []
        : Object.keys(lateralOpt).map((k, i) => {
            ...
            return sql<SQL>` LEFT JOIN LATERAL (${subQ}) AS "ljoin_${raw(
              String(i)
            )}" ON true`;
          });
  ...
  const rowsQuery = sql<
      SQL,
      any
    >`SELECT ${allColsSQL} AS result FROM ${table}${tableAliasSQL}${lateralSQL}${whereSQL}${orderSQL}${limitSQL}${offsetSQL}`,
    query =
      mode !== SelectResultMode.Many
        ...
        : // we need the aggregate to sit in a sub-SELECT in order to keep ORDER and LIMIT working as usual
          sql<
            SQL,
            any
          >`SELECT coalesce(jsonb_agg(result), '[]') AS result FROM (${rowsQuery}) AS ${raw(
            `"sq_${aliasedTable}"`
          )}`;
...
export interface SelectOneSignatures {
  ...
  >(
    ...
    FullSelectReturnTypeForTable<T, C, L, E, SelectResultMode.One>
  >;
}
